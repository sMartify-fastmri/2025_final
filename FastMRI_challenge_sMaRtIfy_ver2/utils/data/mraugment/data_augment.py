...
class AugmentationPipeline:
    ...
    def __init__(self, hparams, seed=None):
        ...
        # Initialize with a proper seed for reproducibility
        self.rng = np.random.RandomState(seed)
        
    def seed(self, seed):
        """Set the random seed for this augmentation pipeline"""
        self.rng = np.random.RandomState(seed)
    ...
    def augment_from_kspace(self, kspace, target_size, max_train_size=None):       
        # Ensure input stays on CPU for multiprocessing compatibility
        if kspace.is_cuda:
            kspace = kspace.cpu()
        ...
        # Ensure all operations stay on CPU during data loading
        target_tensor = rss_complex(im).unsqueeze(0)
        if target_tensor.is_cuda:
            target_tensor = target_tensor.cpu()
        target = center_crop(target_tensor, target_size[0], target_size[1]).squeeze(0)
        
        kspace = fft2c(im)
        # Ensure output stays on CPU
        if kspace.is_cuda:
            kspace = kspace.cpu()
        if target.is_cuda:
            target = target.cpu()
...
class DataAugmentor:
    ...
    def __init__(self, hparams, current_epoch_fn, seed=None):
        ...
        if self.aug_on:
            # Pass seed to the augmentation pipeline
            aug_seed = seed + 1000 if seed is not None else None
            self.augmentation_pipeline = AugmentationPipeline(hparams, seed=aug_seed)
        self.max_train_resolution = hparams.max_train_resolution
        
    def seed_pipeline(self, seed):
        """Set the random seed for the augmentation pipeline"""
        if self.aug_on and hasattr(self, 'augmentation_pipeline'):
