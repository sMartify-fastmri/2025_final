...
def center_crop(data, height, width):
    """
    Center crop the data to the specified shape.
    
    Parameters:
    - data: numpy array or tensor of shape (C, H, W)
    - height: desired output height
    - width: desired output width
    
    Returns:
    - cropped tensor (device placement handled by caller)
    """
    # Convert to PyTorch tensor (keep on CPU to avoid CUDA multiprocessing issues)
